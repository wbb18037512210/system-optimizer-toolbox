...
- **系统工具快捷入口**（左侧按功能分区）：4 个分组以 `LabelFrame` 折叠呈现——
  ...
  - **⚡ 一键优化**（15 项 2 列，需管理员，每项二次确认、全部可逆）：清理 DNS、高性能 / 卓越电源、快速启动、关闭 SysMain、关闭传递优化、关闭搜索索引、关闭透明动画、关闭遥测、关闭休眠、关闭防火墙、关闭 Defender、关闭 UAC、关闭系统还原、关闭 Win 更新。
- **可清理选项紧凑化**：右侧 Treeview 用 `Cleanup.Treeview` 样式（行高 20、字号 9pt，固定 12 行可视），列宽收紧且不横向拉伸（勾选 22 / 项目 148 / 位置 180 / 已占用 78，detail 列 18 字符带省略号），整体不再撑满整列，而是左侧对齐的紧凑方块；"选择/统计"与"操作"区也同步改为左对齐，与清理列表宽度协调，30 项可清理项目通过滚动条查看，布局无溢出、无错位。
- **运行日志面板**：底部实时显示操作结果。

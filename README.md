...
- **顶部 4 分区功能区（横向并排）+ 底部清理项目（占满全宽）**：彻底重排布局——
  - **顶部一行**：`grid` 4 列 `weight=1 uniform="cols"`，4 个 LabelFrame 等宽并排，所有按钮文字完整可见无截断：
    - **🖥 Windows 系统工具**（9 项 2 列 `width=12`）：控制面板、任务管理器、卸载程序、磁盘清理、系统信息、设备管理器、磁盘管理、服务、上帝模式；
    - **🧩 优化与卸载面板**（6 项 2 列 `width=12`）：卸载预装、深度优化、GPU 优化、电源/性能、启动项、Duck 全功能；
    - **🌐 外部工具**（2 项单列 `width=18`）：Win10 优化、360 联网助手；
    - **⚡ 一键优化**（15 项 2 列 `width=15`）：DNS 缓存、高性能/卓越电源、快速启动、禁用 SysMain、关传递优化、关搜索索引、关透明动画、关遥测、关休眠、关防火墙、关 Defender、关 UAC、关系统还原、关 Win 更新。**高危提示置于分组顶部**（红字加粗 + 自动换行），进入即见；
  - **中部全宽**：可清理项目面板（占满整行 1020px），不再受限于右列；
  - **底部全宽**：运行日志（4 行紧凑）。
  - 窗口尺寸 `1020x780`，可在 1080P 屏幕轻松放下，无左右空白带。
- **可清理项目面板**：Treeview 用 `Cleanup.Treeview` 样式（行高 20、字号 9pt，固定 12 行可视），列宽按全宽重新分配（勾选 28 / 项目 200 / 位置 stretch / 已占用 110），`位置`列随窗口缩放自动撑满；detail 列 18 字符带省略号；30 项可清理项目通过滚动条查看，布局无溢出、无错位。统计文字改用品牌绿 `#0a7c2f` 而非粗黑。
